// Lancer Helios Tech Tree Calculator
...
const Map<int, Map<String, int>> blazingChargeData = {
...
const Map<int, Map<String, int>> blazingArmorData = blazingChargeData;
...
const Map<int, Map<String, int>> blazingLanceData = {
...
const Map<int, Map<String, int>> blazingGuardianData = blazingLanceData;

const Map<int, Map<String, int>> heliosData = {
  1: {'crystals': 2236, 'steel': 1000000},
};

class LancerHeliosCalculatorPage extends StatefulWidget {
  const LancerHeliosCalculatorPage({super.key});

  @override
  State<LancerHeliosCalculatorPage> createState() =>
      _LancerHeliosCalculatorPageState();
}

class _LancerHeliosCalculatorPageState
    extends State<LancerHeliosCalculatorPage> {
  ...
  final Map<String, Map<int, Map<String, int>>> techData = {
    ...
    'Blazing Charge': blazingChargeData,
    'Blazing Armor': blazingArmorData,
    'Flame Legion': flameLegionData,
    'Blazing Lance': blazingLanceData,
    'Blazing Guardian': blazingGuardianData,
    'Helios': heliosData,
  };
  ...
  void initState() {
    super.initState();
    AnalyticsService.logPage('LancerHeliosCalculatorPage');
  ...
  Future<void> _saveCloud() async {
    ...
    await CloudSyncService.save('lancers_helios_calculator', data);
  }

  Future<void> _loadCloud() async {
    final data = await CloudSyncService.load('lancers_helios_calculator');
  ...
  Future<void> _recalculateAll() async {
    ...
    await prefs.setInt('helios_lancer_crystals', totalCrystals);
    await prefs.setInt('helios_lancer_steel', totalSteel);
    _saveCloud();
  }
        ...
        children: [
          ...
          if (perTechTotals.containsKey(tech)) ...[
            Padding(
              ...
              child: Text(
                '→ ${formatter.format(perTechTotals[tech]!['crystals'])} Fire Crystal Shards, '
                '${formatter.format(perTechTotals[tech]!['steel'])} Steel',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Lancer Helios'),
        elevation: 0,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Icon(Icons.speed, color: Colors.white, size: 36),
              const SizedBox(width: 12),
              Expanded(
                child: Text('Lancer Tech Tree',
