...
class FullHeliosCalculatorPage extends StatefulWidget {
  const FullHeliosCalculatorPage({super.key});

  @override
  State<FullHeliosCalculatorPage> createState() =>
      _FullHeliosCalculatorPageState();
}

class _FullHeliosCalculatorPageState extends State<FullHeliosCalculatorPage> {
  ...
  final Map<String, Map<int, Map<String, int>>> techData = {
    'FlameSquad': flameSquadData,
    'FlameStrike': blazingChargeData,
    'FlameShield': blazingArmorData,
    'FlameLegion': flameLegionData,
    'FlameTomahawk': blazingLanceData,
    'FlameProtect': blazingGuardianData,
    'Helios': heliosData,
  };
  ...
  void initState() {
    super.initState();
    AnalyticsService.logPage('FullHeliosCalculatorPage');
  ...
  Future<void> _saveCloud() async {
    ...
    await CloudSyncService.save('infantry_helios_calculator', data);
  }

  Future<void> _loadCloud() async {
    final data = await CloudSyncService.load('infantry_helios_calculator');
  ...
  Future<void> _recalculateAll() async {
    ...
    // ✅ Save summary totals for Helios dashboard
    await prefs.setInt('helios_infantry_crystals', totalCrystals);
    await prefs.setInt('helios_infantry_steel', totalSteel);
    _saveCloud();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Infantry Helios'),
        elevation: 0,
