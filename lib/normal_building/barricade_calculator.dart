...
class BarricadeStatsSection extends StatelessWidget {
  final int wood;
  final int meat;
  final int coal;
  final int iron;
...
class BarricadeCalculatorPage extends StatefulWidget {
  const BarricadeCalculatorPage({super.key});

  @override
  _BarricadeCalculatorPageState createState() =>
      _BarricadeCalculatorPageState();
}

class _BarricadeCalculatorPageState extends State<BarricadeCalculatorPage> {
  int _startLevel = 2;
  int _endLevel = 10;
  double _constructionSpeedBonus = 0.0;
  final TextEditingController _speedController = TextEditingController();

  Timer? _cloudTimer; // <-- Add this
  ...
  void initState() {
    super.initState();
    AnalyticsService.logPage('BarricadeCalculatorPage');
    _loadLevels();
  ...
  Future<void> _loadLevels() async {
    ...
    setState(() {
      _startLevel = prefs.getInt('barricade_start') ?? 2;
      _endLevel = prefs.getInt('barricade_end') ?? 10;
      _speedController.text = (prefs.getDouble('barricade_speed_bonus') ?? 0)
          .toString();
  ...
  Future<void> _saveLevels() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt('barricade_start', _startLevel);
    await prefs.setInt('barricade_end', _endLevel);
    await prefs.setDouble('barricade_speed_bonus', _constructionSpeedBonus);
    _saveCloud(); // Save to cloud after saving local
  }

  // --- Cloud Sync ---
  Future<void> _saveCloud() async {
    await CloudSyncService.save('barricade_calculator', {
  ...
  Future<void> _loadCloud() async {
    final data = await CloudSyncService.load('barricade_calculator');
