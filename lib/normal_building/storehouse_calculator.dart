...
class StorehouseStatsSection extends StatelessWidget {
  final int wood;
  final int meat;
  final int coal;
  final int iron;
...
class StorehouseCalculatorPage extends StatefulWidget {
  const StorehouseCalculatorPage({super.key});

  @override
  _StorehouseCalculatorPageState createState() =>
      _StorehouseCalculatorPageState();
}

class _StorehouseCalculatorPageState extends State<StorehouseCalculatorPage> {
  int _startLevel = 2;
  int _endLevel = 10;
  ...
  void initState() {
    super.initState();
    AnalyticsService.logPage('StorehouseCalculatorPage');
    _loadLevels();
    _loadCloud();
  ...
  Future<void> _loadLevels() async {
    ...
    setState(() {
      _startLevel = prefs.getInt('storehouse_start') ?? 2;
      _endLevel = prefs.getInt('storehouse_end') ?? 10;
      _speedController.text = (prefs.getDouble('storehouse_speed_bonus') ?? 0)
          .toString();
  ...
  Future<void> _saveLevels() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt('storehouse_start', _startLevel);
    await prefs.setInt('storehouse_end', _endLevel);
    await prefs.setDouble('storehouse_speed_bonus', _constructionSpeedBonus);
    _saveCloud();
  }

  // --- Cloud Sync ---
  Future<void> _saveCloud() async {
    await CloudSyncService.save('storehouse_calculator', {
  ...
  Future<void> _loadCloud() async {
    final data = await CloudSyncService.load('storehouse_calculator');
